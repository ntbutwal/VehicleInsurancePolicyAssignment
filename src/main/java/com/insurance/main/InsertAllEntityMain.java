...
import com.insurance.repositories.SaveAllEntityRepository;

public class InsertAllEntityMain {
	public static void main(String[] args) {
		ApplicationContext context = new ClassPathXmlApplicationContext("application-context.xml");
		SaveAllEntityRepository rep = (SaveAllEntityRepository) context.getBean("saveAllEntityRepository");
		...
		add.setCustomerEntity(cus);
		veh.setPolicyEntity(pe);
		ade.setPolicyEntity(pe);
		
		rep.saveAllEntities(veh);
		rep.saveAllEntities1(ade);
		rep.saveAllEntities2(add);
		rep.saveAllEntities3(pe);
